...
export default function Component() {
  ...
  useEffect(() => {
    ...
    const nodesRef = ref(database, 'sensorData'); // Path in Firebase should match your Realtime DB path
    onValue(nodesRef, (snapshot) => {
      const data = snapshot.val();
      if (data) {
        const nodesArray = Object.keys(data).map((key, index) => ({
          id: index,
          moisture: data[key].soilMoisture, // Assuming 'soilMoisture' is the field name in your DB
        }));
        setNodes(nodesArray);
      }
    });
  ...
  // Define color based on moisture threshold
